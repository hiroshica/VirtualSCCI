...
{	
	...
	void StoreSample(Sample& dest, int data);
	...
	public:
		...
		Sample	Calc(Sample in);
		Sample	CalcL(Sample in);
		Sample CalcFB(uint fb);
		Sample CalcFBL(uint fb);
		Sample CalcN(uint noise);
		void	Prepare();
		void	KeyOn();
		void	KeyOff();
		void	Reset();
