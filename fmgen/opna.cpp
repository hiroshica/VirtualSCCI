...
void OPNABase::MixSubSL(int activech, Sample** dest)
{
...
inline void OPNABase::MixSubS(int activech, Sample** dest)
{
...
//	リズム音を読みこむ
//
bool OPNA::LoadRhythmSample(HMODULE hInstance)
{
	...
	for (i=0; i<6; i++)
	{
		...
		wavdata += 0x10;
		::memcpy((void*)&whdr, (void*)wavdata, sizeof(whdr));
		wavdata += sizeof(whdr);
		...
		{
			wavdata += fsize;
			::memcpy(&subchunkname, wavdata, 4);
			wavdata += 4;
			::memcpy(&fsize, wavdata, 4);
			wavdata += 4;
		...
		::memcpy(&rhythm[i].sample, wavdata, fsize * 2);
		wavdata += fsize * 2;
...
//	初期化
//
bool OPN2::Init(uint c, uint r, bool ipflag, const char* path)
{
	rate = 8000;
...
{
	return OPNABase::SetRate(c, r, ipflag);
}
